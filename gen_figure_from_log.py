...
def get_acc_from_logfile(log_filename):
    ...
    return trains_accuracy, tests_accuracy, epochs
...
def get_acc_from_logfile2(log_filename):
    trains_accuracy = []
    tests_accuracy = []
    epochs = []
    with open(log_filename, 'r') as f:
        for line in f:
            if 'Epoch' in line and 'Update' not in line and 'used' not in line:
                # print(line)
                epoch = int(line.split()[1]) + 1
                epochs.append(epoch)
            elif 'train acc' in line:
                train_accuracy = to_float(line.split()[2])
                trains_accuracy.append(train_accuracy)
            elif 'test acc' in line:
                test_accuracy = to_float(line.split()[2])
                tests_accuracy.append(test_accuracy)
    assert len(trains_accuracy) == len(tests_accuracy)
    assert len(trains_accuracy) == len(epochs)
    return trains_accuracy, tests_accuracy, epochs
...
if __name__ == '__main__':
    ...
    columns = ['train_acc', 'dev_acc', 'epoch']
    data = {}
    try:
        ret = get_acc_from_logfile(log_filename)
    except Exception as e:
        traceback.print_exc()
        ret = get_acc_from_logfile2(log_filename)
